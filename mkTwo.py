...
# And yet another update for finding files to process 
# so we don't have to specify them all on the command line.
# The command line can be long and unwieldy with many files, and it is easy to forget some.
#
...
parser.add_argument("path", type=str, nargs="+", 
                    help="Path(s) to search for Dinkum binary and PD0 files to convert")
...
for path in args.path:
    if not os.path.isdir(path):
        logging.warning("%s is not a directory, skipping", path)
        continue
    for dirpath, _, filenames in os.walk(path):
        for fn in filenames:
            if re.search(r"[.]pd0$", fn, re.IGNORECASE):
                files.setdefault("pd0", []).append(os.path.join(dirpath, fn))
                continue
            for key in ["d", "e", "m", "n", "s", "t"]:
                if re.search(r"[.]" + key + r"[bc]d$", fn, re.IGNORECASE):
                     ...
                     break
if "d" in files:
    processDBD(files["d"], args) # full density Flight Dinkum Binary files
